...
void nsexec(void)
{
	...
	case JUMP_CHILD:{
		    ...
		    //bail("hyph.getpid:%d", getpid());
            printf("hyph.getpid in nsenter:%d\n", getpid());
			...
			//bail("hyph.child pid:%d", child);
            printf("hyph.getpid in child nsenter:%d\n", getpid());
			if (child < 0)
	...
	case JUMP_INIT:{
            ...
            printf("hyph.getpid in init nsenter:%d\n", getpid());
